"""
133-parameter trained transformer for 10-digit addition.

Based on JackCai's 242p split-subspace architecture with:
  - d_model=5 (tok_dim=2, pos_dim=3), 1 head, head_dim=5
  - vocab=10: all special tokens (PLUS/EQUALS/EOS/PAD) map to digit-0,
    distinguished by position only. Saves 8p over vocab=14.
  - Spiral+correction positional encoding with linear correction
  - Rank-2 attention output projection
  - Frozen EOS special position
  - Tied Q/K with per-head phase rotation

Architecture: 1-layer decoder, d_model=5 (2 tok + 3 pos), 1h, hd=5, ff=2
Training: AdamW (lr=0.02, wd=0.01 adaptive), aggressive WD thresholds
          (Stage2 at val_exact=0.05), grokked at ~15K steps with seed 80085.
"""
...
# Special tokens all map to 0 (distinguished by position)
PLUS_TOKEN = 0
EQUALS_TOKEN = 0
...
class MicroAdder(nn.Module):
    def __init__(self):
        ...
        # Token embedding: only 10 tokens (digits 0-9)
...
def build_model():
    ...
    ckpt_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "checkpoint_133p.pt"
    )
    ...
    metadata = {
        "name": "MicroAdder 133p",
        "author": "Arseniy Zarechnev",
        "params": n_params,
        "architecture": "1L decoder, d=5 (2 tok + 3 pos), 1h, hd=5, ff=2, rank-2 out_proj, tied Q/K + q-phase, vocab=10, RMSNorm, tied output",
    }
...
def add(model, a: int, b: int) -> int:
    ...
    # For vocab=10: read exactly ANSWER_LEN digits (no EOS marker)
    result = 0
