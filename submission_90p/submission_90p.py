"""
90-parameter trained transformer for 10-digit addition.
...
Key innovations:
  ...
  - Spiral+linear correction positions, frozen EOS, tied Q/K + q-phase
...
class MicroAdder(nn.Module):
    def __init__(self):
        ...
        # Carry + special positions
        self.z_hi_pos = nn.Parameter(torch.zeros(1, POS_DIM))
        self.special_pos_learned = nn.Parameter(torch.zeros(2, POS_DIM))
        ...
        # Attention (rank-1 output)
        self.q_proj = nn.Linear(POS_DIM, HEAD_DIM, bias=False)
        self.v_proj = nn.Linear(TOK_DIM, HEAD_DIM, bias=False)
...
def build_model():
    ...
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "checkpoint_90p.pt")
    ...
    # Keys to skip: buffers, shared norms (ln2/ln_f share with ln1)
    SKIP_KEYS = {
    ...
    metadata = {
        ...
        "params": n_params,
        "architecture": "1L decoder, d=5, 1h, hd=5, ff=2, rank-1 out, no FFN bias, parametric tok_emb, vocab=10, shared norms",
    }
