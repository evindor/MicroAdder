"""
62-parameter trained transformer for 10-digit addition.
...
Key innovations:
  ...
  - Parametric token embeddings: 1 arc param (A=B tied); start, stride frozen
  ...
  - Frozen PLUS, EQUALS, and EOS positions (all buffers): saves 9p
  - Frozen spiral amp, phase, slope, offset (all buffers): saves 4p
  - Frozen tok_arc start, stride (buffers): saves 2p
  - Tied tok_arc_A = tok_arc_B (1 param instead of 2): saves 1p vs 66p
...
class MicroAdder(nn.Module):
    def __init__(self):
        ...
        # Parametric token embedding: A=B (tied), start and stride are frozen buffers
        self.tok_arc_A = nn.Parameter(torch.tensor(2.5))
        # CRITICAL: tok_arc_B is aliased to tok_arc_A (same tensor, not a separate parameter)
        ...
        # Carry position: frozen (buffer, NOT parameter) -- holds non-zero from training
    ...
    def _get_positions(self, T):
        ...
        # Build special_pos: [PLUS, EQUALS, EOS] = [frozen, frozen, frozen]
...
def build_model():
    ...
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "checkpoint_62p.pt")
    ...
    # tok_arc_B (aliased to tok_arc_A, same tensor)
    SKIP_KEYS = {
        ...
        "_plus_pos",
        "_eos_pos",
        # tok_arc_B is aliased to tok_arc_A -- loading tok_arc_A handles both
        "tok_arc_B",
    }
    ...
    metadata = {
        ...
        "architecture": (
            ...
            "parametric tok_emb (A=B tied; start,stride frozen), vocab=10, shared norms, tied V/O, "
            "no pos correction, frozen z_hi, frozen PLUS+EQUALS+EOS, "
