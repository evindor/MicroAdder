...
Sequence layout (34 tokens):
    X_0..X_9  PLUS  Y_0..Y_9  EQUALS  Z_0..Z_10  EOS

All special tokens (PLUS, EQUALS, EOS) map to digit-0 since vocab_size=10.
Positions are distinguished by positional encoding only.
"""
...
VOCAB_SIZE = 10  # digits 0-9 only; specials map to 0
MAX_DIGITS = 10
ANSWER_LEN = 11  # 10 digits + 1 carry
SEQ_LEN = 34     # 10 + 1 + 10 + 1 + 11 + 1
PROMPT_LEN = 22  # X(10) + PLUS(1) + Y(10) + EQ(1)

# Special token IDs (all map to digit 0 for vocab=10)
PLUS_ID = 0
EQUALS_ID = 0
EOS_ID = 0
...
EQ_POS = 21
Z_START = 22
Z_HI_POS = 32  # carry digit
EOS_POS = 33

# ── Position source mapping ─────────────────────────────────────────────
# Source types: 0=digit (shared across X/Y/Z), 1=z_hi (carry), 2=special
POS_SOURCES: List[int] = []
POS_INDICES: List[int] = []

for _i in range(MAX_DIGITS):       # X_0..X_9 -> digit_pos[0..9]
    POS_SOURCES.append(0)
    POS_INDICES.append(_i)
POS_SOURCES.append(2)              # PLUS -> special[0]
POS_INDICES.append(0)
for _i in range(MAX_DIGITS):       # Y_0..Y_9 -> digit_pos[0..9]
    POS_SOURCES.append(0)
    POS_INDICES.append(_i)
POS_SOURCES.append(2)              # EQUALS -> special[1]
POS_INDICES.append(1)
for _i in range(MAX_DIGITS):       # Z_0..Z_9 -> digit_pos[0..9]
    POS_SOURCES.append(0)
    POS_INDICES.append(_i)
POS_SOURCES.append(1)              # Z_10 (carry) -> z_hi[0]
POS_INDICES.append(0)
POS_SOURCES.append(2)              # EOS -> special[2]
POS_INDICES.append(2)
...
def encode_number(n: int, num_digits: int = MAX_DIGITS) -> List[int]:
    """Encode integer as LSB-first digit tokens, zero-padded to num_digits."""
    digits = []
...
def decode_answer(tokens: List[int]) -> int:
    ...
    For vocab=10, reads exactly ANSWER_LEN digits (no distinct EOS marker).
    """
    result = 0
...
def make_example(a: int, b: int) -> Tuple[List[int], List[int]]:
    """Create (input_ids, targets) for a + b with teacher forcing.

    input_ids:  tokens[0:33]  (teacher-forced input, length 33)
    targets:    tokens[1:34]  with -100 mask on prompt positions (length 33)
    """
    s = a + b
    ...
    tokens = x + [PLUS_ID] + y + [EQUALS_ID] + z + [EOS_ID]
...
def _sample_carry_example(
    ...
    """Generate a carry-focused addition example.

    Four patterns ensure the model sees abundant carry propagation:
    - single: one position triggers a carry
    - chain: long carry chain (999...9 + small)
    - place: single non-zero digit at one position
    - boundary: near a power of 10
    """
