"""
203-parameter trained transformer for 10-digit addition.

Based on JackCai's 242p split-subspace architecture with:
  - Spiral+correction positional encoding (saves 16p vs learned)
  - Rank-2 attention output projection (saves 12p vs full-rank)
  - Linear positional correction (saves 8p vs per-position)
  - Frozen EOS special position (saves 3p)
...
def build_model():
    ...
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "checkpoint_203p.pt")
    ...
    metadata = {
        "name": "MicroAdder 203p",
        "author": "Arseniy Zarechnev",
        "params": n_params,
        "architecture": "1L decoder, d=6 (3 tok + 3 pos), 2h, hd=3, ff=2, rank-2 out_proj, RMSNorm, tied output",
        "tricks": [
