...
class ModelConfig:
    ...
    pos_correction_mode: str = (
        "full"  # "full" (10 params) | "linear" (2 params) | "none" (0 params)
    )
    ...
    q_phase: bool = (
        False  # Add learnable per-head phase rotation to Q (for tied Q/K asymmetry)
    )
    share_layers: bool = False  # Universal transformer style
    norm_mode: str = "full"  # "full" (18p) | "shared" (6p) | "scalar" (1p) | "fixed" (0p) | "no_ln2" (12p)
    tie_vo: bool = (
        False  # Tie v_proj.weight = head_proj.weight.T (saves tok_dim*head_dim params)
    )
    freeze_pad: bool = (
        False  # Freeze PAD token embedding to zero (saves tok_dim params)
    )
    freeze_z_hi: bool = (
        False  # Freeze z_hi carry position to zero (saves pos_dim params)
    )
    freeze_spiral: str = (
        ""  # Comma-separated spiral params to freeze: "slope,offset" etc
    )
    spiral_init_amp: float = 1.0  # Init/frozen value for spiral_amp
    spiral_init_phase: float = 0.0  # Init/frozen value for spiral_phase
    spiral_init_slope: float = (
        -1.0
    )  # Init/frozen value for spiral_slope (-1 = use default 1/(MAX_DIGITS-1))
    spiral_init_offset: float = 0.0  # Init/frozen value for spiral_offset
    ...
    qk_dim: int = (
        0  # 0 = use head_dim for Q/K; >0 = separate Q/K dim (decouples from V dim)
    )
    q_proj_rank: int = (
        0  # 0 = full rank q_proj; >0 = low-rank factorization (saves params)
    )
    ...
    v_section_mode: str = "none"  # "none" | "fixed_sign" | "learned"
    v_gain_x_init: float = 1.0
    v_gain_y_init: float = -1.0
...
class ToeplitzLinear(nn.Module):
    ...
    def _build_weight(self) -> torch.Tensor:
        ...
        row_offsets = torch.arange(
            self.out_features - 1, -1, -1, device=self.vals.device
        )
...
class SplitAttention(nn.Module):
    ...
    def __init__(self, cfg: ModelConfig):
        ...
        self.v_section_mode = cfg.v_section_mode

        # qk_dim: separate Q/K dimension from V/output dimension
        # When qk_dim > 0, Q/K project to n_heads*qk_dim while V stays at n_heads*head_dim.
        # This allows removing dead Q/K dimensions without affecting the V/output pathway.
        self.qk_dim = cfg.qk_dim if cfg.qk_dim > 0 else cfg.head_dim
        self.qk_inner_dim = cfg.n_heads * self.qk_dim
        ...
        self.kv_qk_inner_dim = self.num_kv_heads * self.qk_dim
        ...
        if cfg.q_proj_rank > 0:
            self.q_proj = LowRankLinear(qk_in_dim, self.qk_inner_dim, cfg.q_proj_rank)
        elif cfg.q_proj_mode == "toeplitz":
            self.q_proj = ToeplitzLinear(qk_in_dim, self.qk_inner_dim)
        else:
            self.q_proj = nn.Linear(qk_in_dim, self.qk_inner_dim, bias=False)
        if not cfg.tie_qk:
            self.k_proj = nn.Linear(qk_in_dim, self.kv_qk_inner_dim, bias=False)
        else:
        ...
        # Optional operand-role signal in value path.
        # X section and Y section can have different gains/signs before V projection.
        if self.v_section_mode not in ("none", "fixed_sign", "learned"):
            raise ValueError(f"Unknown v_section_mode: {self.v_section_mode}")
        if self.v_section_mode != "none":
            x_mask = torch.zeros(cfg.max_seq_len, dtype=torch.bool)
            y_mask = torch.zeros(cfg.max_seq_len, dtype=torch.bool)
            x_mask[X_START : X_START + MAX_DIGITS] = True
            y_mask[Y_START : Y_START + MAX_DIGITS] = True
            self.register_buffer("_v_x_mask", x_mask)
            self.register_buffer("_v_y_mask", y_mask)
            if self.v_section_mode == "learned":
                self.v_gain_x = nn.Parameter(torch.tensor(cfg.v_gain_x_init))
                self.v_gain_y = nn.Parameter(torch.tensor(cfg.v_gain_y_init))

        if cfg.attn_out_rank > 0:
            self.out_proj = LowRankLinear(
                self.inner_dim, cfg.d_model, cfg.attn_out_rank
            )
        else:
    ...
    def _repeat_kv(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return (
            x.unsqueeze(2)
            .expand(B, H, self.kv_repeat, T, D)
            .reshape(B, H * self.kv_repeat, T, D)
        )

    def _value_section_scales(
        self, T: int, device: torch.device, dtype: torch.dtype
    ) -> torch.Tensor:
        """Return per-position value gains, shape (1, T, 1)."""
        if self.v_section_mode == "none":
            return torch.ones(1, T, 1, device=device, dtype=dtype)

        scales = torch.ones(T, device=device, dtype=dtype)
        y_mask = self._v_y_mask[:T]
        if self.v_section_mode == "fixed_sign":
            scales[y_mask] = -1.0
        else:
            x_mask = self._v_x_mask[:T]
            scales[x_mask] = self.v_gain_x.to(device=device, dtype=dtype)
            scales[y_mask] = self.v_gain_y.to(device=device, dtype=dtype)
        return scales.view(1, T, 1)
    ...
    def forward(self, x: torch.Tensor, v_weight: torch.Tensor = None) -> torch.Tensor:
        ...
        if self.v_section_mode != "none":
            x_tok_v = x_tok * self._value_section_scales(T, x_tok.device, x_tok.dtype)
        else:
            x_tok_v = x_tok

        q = self.q_proj(x_qk)
        if self.tie_qk:
            # For GQA + tied: take first kv_qk_inner_dim outputs as K
            k = self.q_proj(x_qk)[:, :, : self.kv_qk_inner_dim]
        else:
            k = self.k_proj(x_qk)
        if v_weight is not None:
            ...
            v = self.v_proj(x_tok_v)

        # Q/K use qk_dim; V uses head_dim (may differ when qk_dim is set)
        q = q.view(B, T, self.n_heads, self.qk_dim).transpose(1, 2)
        k = k.view(B, T, self.num_kv_heads, self.qk_dim).transpose(1, 2)
        ...
        if self.use_alibi:
            ...
            alibi_bias = (
                slopes.view(1, self.n_heads, 1, 1) * self._alibi_rel_pos[:, :, :T, :T]
            )
            att = att + alibi_bias
...
class OffsetAttention(nn.Module):
    ...
    def _compute_attn_bias(self, T: int) -> torch.Tensor:
        ...
        # Shape: (T, T)
        offset = digit_idx.unsqueeze(0) - digit_idx.unsqueeze(
            1
        )  # (T, T) — offset[i,j] = digit[j] - digit[i]
...
class MicroAdder(nn.Module):
    def __init__(self, cfg: ModelConfig):
        ...
        if cfg.tok_emb_mode == "parametric":
            ...
            frozen_arc = (
                set(cfg.freeze_tok_arc.split(",")) if cfg.freeze_tok_arc else set()
            )
            for name, init_val in [
                ("A", 2.5),
                ("B", 2.5),
                ("start", -1.2),
                ("stride", 0.29),
                ...
                if name in frozen_arc:
                    ...
                    setattr(
                        self, f"tok_arc_{name}", nn.Parameter(torch.tensor(init_val))
                    )
        ...
        self.register_buffer(
            "_pos_sources",
            torch.tensor(POS_SOURCES, dtype=torch.long),
            persistent=False,
        )
        self.register_buffer(
            "_pos_indices",
    ...
    def _init_positions(self, cfg: ModelConfig) -> None:
        if cfg.pos_mode == "zero":
            ...
            frozen_spiral = (
                set(cfg.freeze_spiral.split(",")) if cfg.freeze_spiral else set()
            )
            default_slope = 1.0 if cfg.pos_dim == 2 else 1.0 / max(1, MAX_DIGITS - 1)
            init_vals = {
                "amp": cfg.spiral_init_amp,
                "phase": cfg.spiral_init_phase,
                "slope": default_slope
                if cfg.spiral_init_slope < 0
                else cfg.spiral_init_slope,
                "offset": cfg.spiral_init_offset,
            }
            for name in ["amp", "phase", "slope", "offset"]:
                val = init_vals[name]
                if name in frozen_spiral:
                    self.register_buffer(f"spiral_{name}", torch.tensor(val))
                else:
                    setattr(self, f"spiral_{name}", nn.Parameter(torch.tensor(val)))
            ...
            if cfg.pos_correction_mode == "full":
                ...
                raise ValueError(
                    f"Unknown pos_correction_mode: {cfg.pos_correction_mode}"
                )
        else:
    ...
    def _get_special_pos(self) -> torch.Tensor:
        ...
        if cfg.freeze_special == "none":
            ...
            return torch.cat(
                [self._plus_pos, self.special_pos_equals, self._eos_pos], dim=0
            )
        else:  # all
        ...
        if cfg.tok_emb_mode != "parametric" and cfg.token_init == "spiral":
            with torch.no_grad():
                ...
                for d in range(min(10, cfg.vocab_size)):
                    ...
                    lin = d / 9.0
                    if cfg.tok_dim > 0:
                        self.tok_emb.weight[d, 0] = c
                    if cfg.tok_dim > 1:
                        self.tok_emb.weight[d, 1] = s
                    if cfg.tok_dim > 2:
                        self.tok_emb.weight[d, 2] = lin
        ...
        for block in self.blocks:
            ...
            if self.cfg.ffn_dim > target_cfg.ffn_dim:
                ...
                if block.ffn.fc2.bias is not None:
                    l1 = (
                        l1 + block.ffn.fc2.bias.abs().sum()
                    )  # full bias since output dim unchanged

            # Norm scaffold: difference-based (drives ln2, ln_f toward ln1)
            if (
                self.cfg.norm_mode != "shared"
                and target_cfg.norm_mode == "shared"
                and hasattr(block, "ln2")
                and block.has_ln2
    ...
    def prune_scaffold(self, target_cfg: "ModelConfig") -> None:
        ...
        for block in self.blocks:
            ...
            if self.cfg.ffn_dim > target_cfg.ffn_dim:
                ...
                new_fc1 = nn.Linear(old_fc1.in_features, target_dim, bias=has_bias).to(
                    old_fc1.weight.device
                )
                new_fc2 = nn.Linear(target_dim, old_fc2.out_features, bias=has_bias).to(
                    old_fc2.weight.device
                )
                with torch.no_grad():
