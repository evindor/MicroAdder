"""
65-parameter trained transformer for 10-digit addition.

Architecture: 1-layer decoder, d_model=5 (2 tok + 3 pos), 1 head, hd=5, ff=2
Key innovations (same as 66p PLUS):
  - tie_tok_arc_ab: tok_arc_A = tok_arc_B (circular arc, saves 1p vs 66p)
  ...
  - Parametric token embeddings: 1 arc param (A=B) learned; start, stride frozen
  ...
  - Frozen PLUS and EOS positions (only EQUALS learned): saves 6p
...
class MicroAdder(nn.Module):
    def __init__(self):
        ...
        # Parametric token embedding: A=B (tied), start/stride frozen
        self.tok_arc_A = nn.Parameter(torch.tensor(2.5))
        self.tok_arc_B = self.tok_arc_A  # TIED: B is the same Parameter as A
...
def build_model():
    ...
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "checkpoint_65p.pt")
    ...
    SKIP_KEYS = {
        ...
        "_plus_pos",
        "_eos_pos",
        # tok_arc_B is tied to tok_arc_A, skip the duplicate
        "tok_arc_B",
    }
    ...
    metadata = {
        ...
        "architecture": (
            ...
            "no pos correction, frozen z_hi, frozen PLUS+EOS, "
