"""
72-parameter trained transformer for 10-digit addition.
...
Key innovations:
  ...
  - Frozen z_hi carry position (buffer, not parameter): saves 3p vs 78p
  - Frozen PLUS and EOS positions (only EQUALS learned): saves 3p vs 78p
...
def build_model():
    ...
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "checkpoint_72p.pt")
    ...
    # and position correction params (not used in 72p model)
    SKIP_KEYS = {
    ...
    metadata = {
        ...
        "params": n_params,
        "architecture": "1L decoder, d=5, 1h, hd=5, ff=2, rank-1 out, no FFN bias, parametric tok_emb, vocab=10, shared norms, tied V/O, no pos correction, frozen z_hi, frozen PLUS+EOS",
    }
