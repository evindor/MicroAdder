"""
100-parameter trained transformer for 10-digit addition.
...
Training: AdamW (lr=0.02, wd=0.01 adaptive), aggressive WD thresholds
          (Stage2 at val_exact=0.05), grokked at ~36K steps with seed 42.
...
def build_model():
    ...
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "checkpoint_100p.pt")
    ...
    metadata = {
        "name": "MicroAdder 100p",
        "author": "Arseniy Zarechnev",
        "params": n_params,
        "architecture": "1L decoder, d=5, 1h, hd=5, ff=2, rank-1 out, no FFN bias, parametric tok_emb, vocab=10",
    }
