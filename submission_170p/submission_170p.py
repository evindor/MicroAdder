"""
170-parameter trained transformer for 10-digit addition.
...
Based on JackCai's 242p split-subspace architecture with:
  ...
  - Tied Q/K with per-head phase rotation (saves 16p vs separate K projection)
  - tok_dim=2, pos_dim=4 reshape (saves 17p vs tok_dim=3, pos_dim=3)

Architecture: 1-layer decoder, d_model=6 (2 tok + 4 pos), 2 heads, hd=3, ff=2
Training: AdamW (lr=0.02, wd=0.01 adaptive), cosine decay, carry-focused curriculum,
          grokked at seed 80085 in ~15K steps.
"""
...
class MicroAdder(nn.Module):
    def __init__(self):
        ...
        # No k_proj — K reuses q_proj output (tied Q/K)
    ...
    def _get_digit_positions(self):
        ...
        # dim 3 stays zero in base (learned via z_hi_pos and special_pos)
