"""
226-parameter trained transformer for 10-digit addition.

Based on JackCai's 242p split-subspace architecture with spiral+correction
positional encoding replacing learned positions, saving 16 parameters.

Architecture: 1-layer decoder, d_model=6 (3 tok + 3 pos), 2 heads, hd=3, ff=2
Training: AdamW (lr=0.02, wd=0.01), cosine decay, carry-focused curriculum,
          grokked at ~78K steps (seed 420).
"""
...
class MicroAdder(nn.Module):
    def __init__(self):
        ...
        # Token embedding (spiral-initialized)
        self.tok_emb = nn.Embedding(VOCAB_SIZE, TOK_DIM)

        # Spiral+correction positional encoding (14 params vs 30 for learned)
        ...
        self.pos_correction = nn.Parameter(torch.zeros(MAX_DIGITS))

        # Carry position + special token positions
        self.z_hi_pos = nn.Parameter(torch.zeros(1, POS_DIM))
        self.special_pos = nn.Parameter(torch.zeros(3, POS_DIM))
    ...
    def _get_digit_positions(self):
        ...
        return base * (1.0 + self.pos_correction).unsqueeze(1)

    def _get_positions(self, T):
        digit_pos = self._get_digit_positions()
        tables = [digit_pos, self.z_hi_pos, self.special_pos]
    ...
    def forward(self, idx):
        ...
        k = self.k_proj(h[:, :, TOK_DIM:]).view(B, T, N_HEADS, HEAD_DIM).transpose(1, 2)
...
def build_model():
    ...
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "checkpoint_226p.pt")
    ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)

    # Remap keys from training checkpoint (blocks.0.attn.* -> flat names)
    KEY_MAP = {
        "blocks.0.attn.q_proj.weight": "q_proj.weight",
        "blocks.0.attn.k_proj.weight": "k_proj.weight",
        "blocks.0.attn.v_proj.weight": "v_proj.weight",
        "blocks.0.attn.out_proj.weight": "out_proj.weight",
    ...
    for k, v in raw.items():
        if k in ("blocks.0.attn.causal_mask", "_pos_sources", "_pos_indices"):
            continue  # skip buffers, model recreates them
    ...
    metadata = {
        "name": "MicroAdder 226p (spiral-correct)",
        "author": "Arseniy Zarechnev",
        "params": n_params,
        "architecture": "1L decoder, d=6 (3 tok + 3 pos), 2h, hd=3, ff=2, RMSNorm, tied output",
        "tricks": [
            "Split-subspace attention: Q,K from pos dims; V from tok dims",
            "Spiral+correction positional encoding (14 params vs 30 learned)",
            "Shared XYZ positions: X[i]=Y[i]=Z[i]",
            "Tied output head: Linear(6->3) @ tok_emb.T",
            "RMSNorm (no bias)",
            "Carry-focused curriculum: 30% carry-heavy examples, faded after tok_acc > 0.7",
            "LSB-first digit ordering",
            "Grokking via AdamW (lr=0.02, wd=0.01), cosine decay",
        ],
    }
