"""
78-parameter trained transformer for 10-digit addition.
...
Key innovations:
  ...
  - Spiral positions (no linear correction), frozen EOS, tied Q/K + q-phase
  ...
  - No position correction: saves 2p vs 80p model
...
def build_model():
    ...
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "checkpoint_78p.pt")
    ...
    # and position correction params (not used in 78p model)
    SKIP_KEYS = {
    ...
    metadata = {
        ...
        "params": n_params,
        "architecture": "1L decoder, d=5, 1h, hd=5, ff=2, rank-1 out, no FFN bias, parametric tok_emb, vocab=10, shared norms, tied V/O, no pos correction",
    }
