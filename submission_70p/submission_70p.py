"""
70-parameter trained transformer for 10-digit addition.
...
Key innovations:
  ...
  - Frozen spiral_offset AND spiral_phase (buffers): saves 2p vs 71p
...
class MicroAdder(nn.Module):
    def __init__(self):
        ...
        # spiral_offset AND spiral_phase are BUFFERS (frozen, not parameters)
        # They may hold non-zero values from warm-starting, loaded from checkpoint.
        ...
        # Carry position: frozen (buffer, NOT parameter) -- may hold non-zero from training
...
def build_model():
    ...
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "checkpoint_70p.pt")
    ...
    SKIP_KEYS = {
        ...
        "_plus_pos",
        "_eos_pos",
        # NOTE: z_hi_pos, spiral_offset, spiral_phase are NOT skipped -- they are
        # buffers but may hold non-zero values from warm-starting.
    }
    ...
    metadata = {
        ...
        "architecture": (
            ...
            "parametric tok_emb, vocab=10, shared norms, tied V/O, "
            "no pos correction, frozen z_hi, frozen PLUS+EOS, "
            "frozen spiral_offset+spiral_phase"
        ),
    }
